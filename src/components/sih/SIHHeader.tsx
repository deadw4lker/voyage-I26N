    ...
    <header className="border-b border-[#1e2b45] bg-[#0c1527] px-4 py-2.5 sm:px-5">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-2">
          ...
          <nav aria-label="Views" className="flex items-center gap-0.5 rounded-lg border border-[#24344f] bg-[#0a1222] p-0.5">
            {(['operations', 'alerts'] as const).map((v) => (
              <button
                ...
                className={`flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-[12px] font-medium capitalize transition-colors ${
                  view === v ? 'bg-[#1c2f4f] text-slate-100' : 'text-[#8b98ad] hover:text-slate-200'
                }`}
              >
                {v}
